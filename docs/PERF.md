...
1. **自适应码率/帧率**（改最小，收益最大）——✅ 已在 v0.6.x 实现（`getStats()` 丢包检测 + 升降档）
2. **SFU**（多人/语音刚需，架构级，改动大）——待实施
3. 内容感知编码 + 音频 FEC/DTX——部分（`contentHint` 已在 v0.6 使用；FEC/DTX 待做）
4. **可观测性**——✅ 已在 v0.8 实现：`GET /api/stats` + 生命周期结构化日志
