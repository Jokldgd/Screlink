...
- **统一房间入口（语音版本）**：页面不再区分「共享屏幕 / 加入观看」两个入口，改为输入房间号即进入——首个进入者自动创建房间，后续成员输入同一房间号加入。语音与画面共享**共用同一房间号**，房间内既可语音通话也可共享画面
- **全员双向语音（mesh）**：进入房间自动开启麦克风，房间内所有成员两两建立点对点语音连接（WebRTC mesh），双向通话、互不影响；支持一键静音
- **任意成员可共享画面**：动态「共享者」机制——任何成员都可发起/停止画面共享，新共享者发起时自动替换旧共享者（旧共享者收到通知停止推流），成员之间随时切换
- **成员列表**：房间侧边栏显示在线成员、共享状态标记（共享中/成员/自己）
- **双向音量控制**：每个成员行新增「麦克风 / 扬声器」图标，点击展开音量滑条——
  - 麦克风图标：调节自己的发送音量（WebAudio 增益），或某成员的说话音量
  - 扬声器图标：调节每个成员的接收音量（独立记忆，重连不丢失）
- **共享画质与帧率设置**：发起共享前可选 1080p/720p/360p × 15/30/60fps 组合
- **语音设备诊断**：页面加载自动检测麦克风设备，无输入设备时明确提示「只能听」；麦克风失败时区分提示非 HTTPS / 权限被拒 / 无设备 / 设备被占用
- **保留全部既有能力**：自适应码率（丢包驱动动态调档）、观看端清晰度选择（1080p/720p/360p）、控制条自动隐藏、全屏、画质/帧率自由组合等

### 修复

- **画面共享看不到**：共享者向观看者推流的视频连接缺失 `onicecandidate`（ICE 候选从未发送），导致 P2P 打洞失败、连接无法建立——已补齐
- **语音不通**：语音应答方缺失 `flushIce`（offer 前到达的 ICE 候选被丢弃）；新旧成员双向重复建连造成连接冲突；无麦克风成员无法收听他人——均已修复
- **「暂无画面共享」提示不消失**：`.share-prompt` 的 `display:flex` 覆盖了 `hidden` 属性——补充 `[hidden]` 规则使隐藏真正生效
- **进入房间偶发「房间不存在或已结束」**：多人并发输入同一房间号时，先创建者成功、其余人自动创建撞「房间号被占用」而被卡在门外——新增 `room-taken → 自动加入` 双向兜底（join 失败自动创建、create 冲突自动加入）
- **语音不可用提示模糊**：原统一提示「麦克风不可用」，现按根因分类提示（非 HTTPS / 权限拒绝 / 无设备 / 设备占用 / 无符合设备）
